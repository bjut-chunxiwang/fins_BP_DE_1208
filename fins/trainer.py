...
class Trainer:
    ...
    def make_batch_data(self, batch):
        ...
        # Generate vector_32d
        extra_param1 = batch['extra_param1'].to(self.device)
        extra_param2 = batch['extra_param2'].to(self.device)
        extra_param3 = batch['extra_param3'].to(self.device)
        vector_32d = torch.cat([extra_param1.unsqueeze(1).repeat(1, 64),
                                extra_param2.unsqueeze(1).repeat(1, 64)], dim=-1)
        extra_param3 = extra_param3.unsqueeze(1)  # 变为 [16, 1]
        ...
        for epoch in range(self.args.resume_step, self.config.num_epochs):
            ...
            for i, batch in enumerate(self.train_data):
                ...
                (
                    ...
                    batch_stochastic_noise,
                    batch_noise_condition,
                    vector_32d,  # 新增 vector_32d
                    extra_param3,
                ) = self.make_batch_data(batch)

                # Model forward
                predicted_rir = self.model(
                    reverberated_source_with_noise, batch_stochastic_noise, batch_noise_condition, vector_32d, extra_param3
                )
            ...
            if (epoch + 1) % self.config.checkpoint_interval == 0:
                ...
                torch.save(state_dicts, os.path.join(self.model_checkpoint_dir, f"epoch-.pt"))
    ...
    def plot(self, batch, nth_batch, epoch):
        ...
        #
        # # 假设在配置中指定了保存文件夹
        # self.config.save_folder = 'saved_images'
        #
        # # 确保保存文件夹存在
        ...
        #     plt.legend()
        #     # 保存图像到指定文件夹
        #     save_path = os.path.join(self.config.save_folder, f"rir_plot_{nth_batch * self.config.batch_size + i}.png")
